...
namespace Ep128 {
  ...
  uint32_t Dave::runOneCycle_()
  {
    ...
    if (dac_mode_left) {
      lval += (chn0_left << 2);
      audio_out[0][0] += (chn0_left << 2);
      if (dac_mode_right) {
        // simplest case: both channels in DAC mode
        rval += (chn0_right << 2);
        audio_out[0][1] += (chn0_right << 2);
      }
      else {
        ...
        {
          rval += chn1_right;
          audio_out[1][1] += (chn1_right << 2);
        }
        if (chn2_state)
        {
          rval += chn2_right;
          audio_out[2][1] += (chn2_right << 2);
        }
        if (chn3_state)
        {
          rval += chn3_right;
          audio_out[3][1] += (chn3_right << 2);
        }
      }
    }
      ...
      {
        lval += chn1_left;
        audio_out[1][0] += (chn1_left << 2);
      }
      if (chn2_state)
      {
        lval += chn2_left;
        audio_out[2][0] += (chn2_left << 2);
      }
      if (chn3_state)
      {
        lval += chn3_left;
        audio_out[3][0] += (chn3_left << 2);
      }
    }
    else {
