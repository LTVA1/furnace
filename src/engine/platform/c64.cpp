...
void DivPlatformC64::tick(bool sysTick) {
  ...
  for (int _i=0; _i<3; _i++) {
    ...
    if (chan[i].std.get_div_macro_struct(DIV_MACRO_ALG)->had && (_i==2 || macroRace)) { // new cutoff macro
      DivInstrument* ins=parent->getIns(chan[i].ins,DIV_INS_C64);
...
int DivPlatformC64::dispatch(DivCommand c) {
  ...
  switch (c.cmd) {
    case DIV_CMD_NOTE_ON: {
      ...
      if (((chan[c.chan].insChanged || ins->std.get_macro(DIV_MACRO_WAVE, true)->len>0) && ins->c64.resetDuty) || chan[c.chan].resetDuty)
      {
